...
let verifiedTokenCache: { token: string; installationId: string; payload: EntitlementPayload } | null = null;
...
async function verifyEntitlement(token: string, installationId: string): Promise<EntitlementPayload> {
	if (verifiedTokenCache?.token === token && verifiedTokenCache.installationId === installationId) {
		const exp = Number(verifiedTokenCache.payload.exp || 0);
		if (exp && Math.floor(Date.now() / 1000) <= exp) return verifiedTokenCache.payload;
		verifiedTokenCache = null;
	}
	...
	verifiedTokenCache = { token, installationId, payload };
	return payload;
}
...
async function persistEntitlement(licenseKey: string, payload: EntitlementPayload, entitlement: string, row: LicenseRow | null) {
	...
	verifiedTokenCache = { token: entitlement, installationId: String(payload.installationId || ''), payload };
