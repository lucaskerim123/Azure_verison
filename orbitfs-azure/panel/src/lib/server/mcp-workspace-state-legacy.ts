...
import { isApexManagedMcpBundleName } from '$lib/server/apex-mcp-integration';
...
export async function projectBundleAssignments(projectId: string) {
	...
	if (error) throw error;
	return (data ?? []).map((r:any) => ({ bundleId:r.bundle_id, name:r.mcp_context_bundles?.name, required:r.required_flag !== false, systemManaged:isApexManagedMcpBundleName(r.mcp_context_bundles?.name) }));
}

export async function saveProjectBundleAssignments(projectId: string, assignments: any[]) {
	const db = getSupabaseAdmin();
	const current=await projectBundleAssignments(projectId);
	const managed=current.filter((item:any)=>item.systemManaged===true);
	const managedIds=new Set(managed.map((item:any)=>String(item.bundleId)));
	const del = await db.from('mcp_project_context_bundles').delete().eq('project_id', projectId);
	if (del.error) throw del.error;
	const userRows=(assignments || []).filter((a:any)=>a?.bundleId&&!managedIds.has(String(a.bundleId))).map((a:any,i:number) => ({ project_id:projectId, bundle_id:a.bundleId, required_flag:a.required !== false, sort_order:i }));
	const rows=[...userRows,...managed.map((a:any,i:number)=>({project_id:projectId,bundle_id:a.bundleId,required_flag:false,sort_order:900+i}))];
...
export async function listContextBundles(workspaceId: string) {
	...
	return (bundles ?? []).map((b:any) => ({ ...b, systemManaged:isApexManagedMcpBundleName(b.name), managedBy:isApexManagedMcpBundleName(b.name)?'apex-knowledge':null, entryCount:(entries.data ?? []).filter((x:any)=>x.bundle_id===b.id).length, dependencyCount:(deps.data ?? []).filter((x:any)=>x.bundle_id===b.id).length }));
}
export async function getContextBundle(workspaceId: string, bundleId: string) {
	...
	return { ...bundle,
		systemManaged:isApexManagedMcpBundleName(bundle.name),managedBy:isApexManagedMcpBundleName(bundle.name)?'apex-knowledge':null,
...
async function assertBundleUserEditable(workspaceId:string,bundleId?:string,name?:string){
	if(name&&isApexManagedMcpBundleName(name))throw Object.assign(new Error('This bundle name is reserved for APEX + Knowledge Setup system context.'),{status:409,code:'CCS_SYSTEM_BUNDLE_RESERVED'});
	if(!bundleId)return;
	const db=getSupabaseAdmin();
	const current=await db.from('mcp_context_bundles').select('name').eq('workspace_id',workspaceId).eq('id',bundleId).maybeSingle();
	if(current.error)throw current.error;
	if(current.data&&isApexManagedMcpBundleName(current.data.name))throw Object.assign(new Error('APEX-managed Knowledge bundles are controlled by Knowledge Setup and cannot be edited directly.'),{status:409,code:'CCS_SYSTEM_BUNDLE_MANAGED'});
}

export async function saveContextBundle(workspaceId:string,user:OrbitUser,body:any,bundleId?:string) {
	const db=getSupabaseAdmin();
	const requestedName=String(body.name||'').trim();
	await assertBundleUserEditable(workspaceId,bundleId,bundleId?undefined:requestedName);
	const payload={workspace_id:workspaceId,name:requestedName,description:String(body.description||''),enabled:body.enabled!==false,updated_at:now()};
	let id=bundleId;
...
export async function deleteContextBundle(workspaceId:string,bundleId:string){
	await assertBundleUserEditable(workspaceId,bundleId);
	const db=getSupabaseAdmin();
...
export async function getPresetBundles(workspaceId:string,projectId?:string|null){
	...
	const result:any={};for(const p of PRESETS) result[p]=(data??[]).filter((x:any)=>x.preset===p).map((x:any)=>{const name=names.find((n:any)=>n.id===x.bundle_id)?.name;return{bundleId:x.bundle_id,name,required:x.required_flag!==false,systemManaged:isApexManagedMcpBundleName(name)};}); return result;
}

export async function savePresetBundles(workspaceId:string,assignments:any,projectId?:string|null){
	const db=getSupabaseAdmin(),t=presetTables(projectId),scopeId=projectId||workspaceId;
	const existing=await getPresetBundles(workspaceId,projectId);
	await db.from(t.bundles).delete().eq(t.scope,scopeId);
	const rows:any[]=[];
	for(const p of PRESETS){
		const managed=(existing?.[p]||[]).filter((a:any)=>a.systemManaged===true);
		const managedIds=new Set(managed.map((a:any)=>String(a.bundleId)));
		const user=(assignments?.[p]||[]).filter((a:any)=>a?.bundleId&&!managedIds.has(String(a.bundleId)));
		user.forEach((a:any,i:number)=>rows.push({[t.scope]:scopeId,preset:p,bundle_id:a.bundleId,required_flag:a.required!==false,sort_order:i}));
		managed.forEach((a:any,i:number)=>rows.push({[t.scope]:scopeId,preset:p,bundle_id:a.bundleId,required_flag:false,sort_order:900+i}));
	}
