...
const fail=(e:any)=>json({error:String(e?.message||'Request failed'),code:String(e?.code||'ADDON_ITEM_ERROR')},{status:Number(e?.status||500)});
...
export async function DELETE({params,cookies}:any){
	try{
		...
		const addon=await saveCloudAddon(id,{
			...
			deployment_url:null,
			config:{},
			runtime:{mode:manifest.runtimeMode||'engine-host',engineMode:defaultEngineMode(id),setupState:'not_started',compute:'vercel',database:'shared-panel',online:false}
		});
		await writeAudit({actorUserId:user.id,action:'addon.uninstall',targetType:'addon',targetId:id,detail:{preservedData:true,libraryRoute:true}});
