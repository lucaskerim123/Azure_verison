...
on conflict(role_key) do update set name=excluded.name,description=excluded.description,rank=excluded.rank,permissions=excluded.permissions,active=excluded.active,sort_order=excluded.sort_order;
...
on conflict(slug) do update set name=excluded.name,description=excluded.description,email=excluded.email,enabled=excluded.enabled,client_can_close=excluded.client_can_close,sort_order=excluded.sort_order,allow_direct_create=excluded.allow_direct_create,min_support_rank=excluded.min_support_rank,updated_at=now();
...
on conflict(set_key,option_key) do update set label=excluded.label,value=excluded.value,active=excluded.active,sort_order=excluded.sort_order;
...
on conflict(id) do update set integration_enabled=excluded.integration_enabled,auto_suspend_overdue=excluded.auto_suspend_overdue,overdue_grace_days=excluded.overdue_grace_days,auto_restore_paid=excluded.auto_restore_paid,ban_blocks_licenses=excluded.ban_blocks_licenses,suspension_blocks_licenses=excluded.suspension_blocks_licenses,updated_at=now();
...
on conflict(address) do update set display_name=excluded.display_name,kind=excluded.kind,active=excluded.active,updated_at=now();
...
on conflict(code) do update set provider=excluded.provider,display_name=excluded.display_name,description=excluded.description,enabled=excluded.enabled,sort_order=excluded.sort_order,supports_recurring=excluded.supports_recurring,supports_refunds=excluded.supports_refunds,supports_partial=excluded.supports_partial,currencies=excluded.currencies,fee_fixed_cents=excluded.fee_fixed_cents,fee_percent=excluded.fee_percent,public_config=excluded.public_config,secret_env_keys=excluded.secret_env_keys,instructions=excluded.instructions,updated_at=now();
